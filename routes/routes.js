...
app.get("/totalUsers", (req, res) => {
  ...
  const selectQuery = `SELECT * FROM mas_user ORDER BY user_id DESC LIMIT ? OFFSET ?`;
...
app.get("/pendingUsers", (req, res) => {
  const selectQuery = "SELECT * FROM mas_user WHERE profile_Image IS NULL OR Emergency_No IS NULL";
